      ...
      {propertiesData &&
        propertiesData.map((property) => (
          <Link 
            to={`/bds`} 
            key={property.id} 
            className="w-full rounded-md bg-white shadow-lg hover:rounded-2xl cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5"
          >
            <img
              src={property.image}
              alt={property.title}
            ...
            <div className="w-full flex items-center justify-between px-2 py-5">
              ...
              <span className="w-8 relative cursor-pointer group flex items-center justify-center h-8 border rounded-lg">
                ...
                <span className="py-1 absolute top-9 opacity-0 group-hover:opacity-100 left-[-50px] text-center w-[130px] px-2 text-sm bg-slate-900 text-white">
                  Bấm để lưu tin
                </span>
              </span>
            </div>
